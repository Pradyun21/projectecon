...
export function PageIntro({ eyebrow, title, text, prominent = false }: { eyebrow:string; title:string; text:string; prominent?:boolean }) { return <section className={`page-intro ${prominent ? "page-intro-prominent" : ""}`}><GradientShapes variant="blue"/><div className="orb orb-one"/><div className="container"><Reveal><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p className="lead">{text}</p></Reveal></div></section>; }
