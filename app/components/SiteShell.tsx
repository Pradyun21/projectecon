...
export function Footer() { return <footer><div className="container footer-grid"><div><Link href="/" className="footer-brand" aria-label="Project Econ home"><BrandLockup /></Link><p>A student-led initiative applying economics to real local problems—one focused concept at a time.</p></div><div><h3>Explore</h3>{[...links, workWithUs].map(l => <Link key={l.href} href={l.href}>{l.label}</Link>)}</div><div><h3>Local to</h3><p>Cabarrus County,<br/>North Carolina</p><Link className="footer-cta" href={workWithUs.href}>Start a conversation <EmojiGlyph emoji="↗️"/></Link></div></div><div className="container footer-bottom"><span>© {new Date().getFullYear()} Project Econ</span><p>Project Econ provides free, student-led, educational recommendations. Results are not guaranteed, and each business or organization is responsible for its own decisions.</p></div></footer> }
