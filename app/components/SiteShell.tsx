  ...
  useEffect(() => {
    ...
    function move(event: PointerEvent) {
      ...
      frame = window.requestAnimationFrame(() => {
        ...
        background.current?.style.setProperty("--pointer-x", `${x.toFixed(2)}px`);
        background.current?.style.setProperty("--pointer-y", `${y.toFixed(2)}px`);
      });
    }
    window.addEventListener("pointermove", move, { passive: true });
    return () => { window.cancelAnimationFrame(frame); window.removeEventListener("pointermove", move); };
  }, []);
  return <div ref={background} className="ambient-background" aria-hidden="true">
...
export function SiteShell({ children }: { children: React.ReactNode }) {
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => entries.forEach(entry => {
      if (!entry.isIntersecting || entry.target.hasAttribute("data-prompted")) return;
      entry.target.setAttribute("data-prompted", "true");
      const delay = Number((entry.target as HTMLElement).dataset.readingDelay ?? 5000);
      timers.set(entry.target, window.setTimeout(() => {
        entry.target.classList.add("is-prompted");
        window.setTimeout(() => entry.target.classList.remove("is-prompted"), 1900);
      }, delay));
    }), { threshold: .7 });
    document.querySelectorAll<HTMLElement>(".hero-prompt, .process-next").forEach(element => observer.observe(element));
    return () => { observer.disconnect(); timers.forEach(timer => window.clearTimeout(timer)); };
  }, []);
  return <BrandIntro><AmbientBackground/><Navbar/><main>{children}</main><Footer/></BrandIntro>;
}
...
export function CallToAction({ title="Could economic analysis provide useful insight?", text="Tell us about the organization, its decisions, and the data available. We’ll begin by understanding the context." }: {title?:string;text?:string}) { return <section className="cta-section"><div className="container"><Reveal className="cta-card"><div><p className="eyebrow light">Start a conversation</p><h2>{title}</h2><p>{text}</p></div><Link className="button button-white" href={workWithUs.href}>Work With Us <EmojiGlyph emoji="↗️"/></Link></Reveal></div></section> }
