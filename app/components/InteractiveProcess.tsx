  ...
  return <section id="process" className="section process-section">
    ...
    <div className="container process-container">
      ...
      <div className="journey-shell">
        <div className="journey-track" aria-hidden="true"><motion.span animate={{ width: `${(active / (processSteps.length - 1)) * 100}%` }} transition={{ duration: .45, ease: [0.22, 1, 0.36, 1] }}/></div>
        <div className="journey-nodes" role="tablist" aria-label="Project Econ process">
          {processSteps.map((step, index) => {
            const Icon = step.icon;
            return <button key={step.title} type="button" role="tab" aria-selected={active === index} aria-controls="journey-detail" className={active === index ? "active" : ""} onClick={() => setActive(index)}>
        ...
        <div id="journey-detail" className="journey-detail" role="tabpanel" aria-live="polite">
          <AnimatePresence mode="wait">
            <motion.div key={selected.title} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: .25 }}>
              <span>Step {active + 1}</span><h3>{selected.title}</h3><p>{selected.detail}</p>
            </motion.div>
          </AnimatePresence>
